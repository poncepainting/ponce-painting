...
export function HeaderAccent({
  ...
  const borderRadius = useAsSubheading 
    ? `rounded-full` 
    : `rounded-full`;
  ...
  const accentClasses = [
    ...
    height,
    borderRadius,
    useAsSubheading ? `px-4 py-1 inline-block ${textColorClass} text-xs font-medium tracking-wide uppercase` : '',
    'mb-4',
    className
  ].filter(Boolean).join(' ');
  ...
  return (
    <div className={`${accentClasses} ${centered ? 'mx-auto' : ''}`}>
      {children}
    </div>
  );
}
